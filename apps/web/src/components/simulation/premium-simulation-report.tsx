...
function EvidenceQuote({ children }: { children: string }) {
  return children === "No strong evidence observed."
    ? <p className="mt-2 text-xs leading-5 text-muted-foreground">{children}</p>
    : <blockquote className="mt-2 border-l-2 border-primary/40 pl-3 text-xs italic leading-5 text-muted-foreground">“{children}”</blockquote>;
}
...
export function PremiumSimulationReport({ sessionId }: { sessionId: string }) {
  ...
  return (
    <article className="mx-auto max-w-6xl space-y-7 print:max-w-none print:space-y-5" aria-label="Premium simulation report">
      <div className="flex flex-wrap items-center justify-between gap-3 print:hidden"><Button asChild variant="ghost" size="sm"><Link href={`/simulation-studio/simulations/${session.simulationId}`}><ArrowLeft />Simulation details</Link></Button><div className="flex gap-2"><Button variant="outline" size="sm" aria-pressed={communicationIntelligenceVisible} onClick={() => setCommunicationIntelligenceVisible(!communicationIntelligenceVisible)}>Communication feedback: {communicationIntelligenceVisible ? "On" : "Off"}</Button><Button variant="outline" size="sm" onClick={() => window.print()}><Printer />Print report</Button></div></div>
      ...
      <section aria-labelledby="capability-snapshot"><div><h2 id="capability-snapshot" className="text-xl font-semibold">Capability Snapshot</h2><p className="mt-1 text-sm text-muted-foreground">Qualitative signals from observed conversation evidence—not a second final score.</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{report.capabilitySnapshot.map((item) => <Card key={item.capability} className="print:break-inside-avoid"><CardContent className="p-4"><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold">{item.capability}</p><Badge variant={item.state === "Needs attention" ? "warning" : item.state === "Not observed yet" ? "secondary" : "outline"}>{item.state}</Badge></div><p className="mt-2 text-xs leading-5 text-muted-foreground">{item.helper}</p></CardContent></Card>)}</div>{communicationIntelligenceVisible && <><h3 className="mt-6 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Communication Quality</h3><div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{report.communicationSnapshot.map((item) => <Card key={item.signal} className="print:break-inside-avoid"><CardContent className="flex items-center justify-between gap-3 p-4"><p className="text-sm font-semibold">{item.signal}</p><Badge variant={item.state === "Needs attention" ? "warning" : item.state === "Not observed yet" ? "secondary" : "outline"}>{item.state}</Badge></CardContent></Card>)}</div></>}</section>
      ...
      <div className="grid gap-5 lg:grid-cols-2"><Card className="print:break-inside-avoid"><CardHeader><CardTitle className="flex items-center gap-2"><CheckCircle2 className="size-5 text-emerald-600" />Strengths</CardTitle><CardDescription>Successful behaviours supported by session evidence.</CardDescription></CardHeader><CardContent className="space-y-5">{report.strengths.length ? report.strengths.map((item) => <div key={item.title}><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-semibold">{item.title}</p><Badge variant="outline">{item.capability}</Badge></div><EvidenceQuote>{item.evidence}</EvidenceQuote></div>) : <p className="text-sm text-muted-foreground">No strong evidence observed. Build one clear, repeatable strength in the next attempt.</p>}</CardContent></Card><Card className="print:break-inside-avoid"><CardHeader><CardTitle className="flex items-center gap-2"><Target className="size-5 text-amber-600" />Missed Opportunities</CardTitle><CardDescription>Specific moments to handle differently in the next practice.</CardDescription></CardHeader><CardContent className="space-y-5">{report.missedOpportunities.map((item) => <div key={item.title}><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-semibold">{item.title}</p><Badge variant="outline">{item.capability}</Badge></div><EvidenceQuote>{item.evidence}</EvidenceQuote><p className="mt-2 text-xs leading-5"><span className="font-semibold">Why it matters:</span> {item.recommendation}</p><p className="mt-2 rounded-lg bg-muted/40 p-3 text-xs leading-5"><span className="font-semibold">Better response:</span> “{item.betterResponse}”</p></div>)}</CardContent></Card></div>
