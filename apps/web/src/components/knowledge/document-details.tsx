...
export function DocumentDetails({ id }: { id: string }) {
  ...
  useEffect(() => {
    let active = true;
    void apiFetch<DocumentDetail>(`/api/documents/${id}`)
      .then((result) => {
  ...
  async function download(path: string, fileName: string) {
    try {
      window.location.assign(await getDocumentDownloadUrl(path, fileName));
    } catch (caught) {
      setError(
        caught instanceof Error
          ? caught.message
  ...
  async function replaceFile(file: File) {
    ...
    try {
      ...
      const input: CreateDocumentVersionInput = {
        ...
        sizeBytes: file.size,
        storagePath,
        notes: `Replaced version ${document.currentVersion}`,
      };
      await apiFetch(`/api/documents/${document.id}/versions`, {
        method: "POST",
      ...
      if (storagePath)
        await removeUploadedFile(storagePath).catch(() => undefined);
      setError(
        caught instanceof Error
          ? caught.message
          : "Unable to replace this document.",
      );
    } finally {
      setPending(false);
  ...
  async function removeDocument() {
    ...
    try {
      ...
      router.replace(
        `/knowledge-studio/knowledge-bases/${document.knowledgeBase.id}`,
      );
    } catch (caught) {
      setError(
        caught instanceof Error
  ...
  if (error && !document)
    return (
      <div className="rounded-xl border bg-card p-8 text-center">
        <h1 className="font-semibold">Document unavailable</h1>
  ...
  return (
    <div className="space-y-7">
      <Button asChild variant="ghost" size="sm" className="-ml-2">
        <Link
          href={`/knowledge-studio/knowledge-bases/${document.knowledgeBase.id}`}
        >
          <ArrowLeft />
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex min-w-0 gap-4">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge
                  variant={
                    document.status === "Ready" ? "success" : "secondary"
                  }
                >
                  {document.status}
                </Badge>
                <ProcessingStatusBadge status={document.processing.status} />
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
              onClick={() =>
                void download(document.storagePath, document.fileName)
              }
            >
              <Download />
              Download
        ...
        <Card>
          ...
          <CardContent className="space-y-4 text-sm">
            ...
            <div className="flex justify-between gap-4">
              ...
              <span className="text-right">{document.uploadedBy.name}</span>
            </div>
            <div className="flex justify-between gap-4">
              ...
              {document.versions.map((version) => (
                ...
                >
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      void download(version.storagePath, version.fileName)
                    }
                  >
                    <Download />
                    Download
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Update display metadata only. The stored file remains unchanged.
            </DialogDescription>
          </DialogHeader>
          <form className="space-y-5" onSubmit={saveMetadata}>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Add context for reviewers and future versions."
              />
            </div>
            <DialogFooter>
              <Button
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This permanently deletes the original file, all stored versions,
              metadata, and notes. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
