...
export async function loadSophiaPromptContext(input: { organizationId: string; learnerId: string; simulationId: string }): Promise<SophiaPromptContext> {
  const simulation = await prisma.simulation.findFirst({
    ...
    include: { persona: true, objectives: { orderBy: { sortOrder: "asc" } }, knowledgeBases: { select: { knowledgeBaseId: true } }, evaluationCriteria: { include: { criterion: { select: { name: true } } } } },
  });
  ...
  const [blueprint, profile, candidates, documentChunks] = await Promise.all([
    ...
    knowledgeBaseIds.length ? prisma.knowledgeChunk.findMany({ where: { status: "ACTIVE", source: { organizationId: input.organizationId, status: "Completed", document: { knowledgeBaseId: { in: knowledgeBaseIds } } } }, select: { chunkNumber: true, text: true, source: { select: { title: true } }, document: { select: { fileName: true } } }, orderBy: [{ sourceId: "asc" }, { chunkNumber: "asc" }], take: getEnv().AI_KNOWLEDGE_SECTION_LIMIT }) : [],
  ]);
